...
def process_img(img, imgDir, Annotation_Json):
    TargetImg = img
...
def Get_Prep_Annotation(imgDir,JsonPath):
    ...
    with Pool(processes=8) as threadpool: # multi threading
        Annotation_Output = threadpool.starmap(process_img, zip(images_file, itertools.repeat(imgDir), itertools.repeat(Annotation_Json)))
...
def Aug_IMGs(in_img_P,in_Annotation_P ,in_Label_2_ClassName,tfs,out_Img_P,out_Annotation_p):
    ...
    for img in tqdm(Imgs_Prepared_to_Trans, desc="IMG NO.", total=len(Imgs_Prepared_to_Trans), colour="green", leave=False):
        # Every img go through every transform
        print(img)
        tf_Count = 1
        for tf in tqdm(tfs, desc="TRF. No.", total=len(tfs), colour="red", leave=False):
            print(img["img_Name"])
            #Augmentation
